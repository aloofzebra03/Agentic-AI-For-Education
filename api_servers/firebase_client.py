...
Firebase Firestore client for:
  ...
  - Per-email daily rate limiting (100 requests/day, resets at midnight IST)
...
Firestore structure:
  ...
  Collection: "rate_limits"
    Document: "{email}:{YYYY-MM-DD}"  ← one doc per user per IST calendar day
      Fields:
        email      : str
...
# ─── Legacy Firestore path constants (kept for any existing references) ───────
_COLLECTION = "auth"
...
def _get_db():
    ...
    if firebase_admin._apps:
        # Already initialized (e.g., in tests or multiple imports)
...
def check_and_increment_rate_limit(
    ...
    push the counter past the limit without detection.

    Document key: "rate_limits/{email}:{YYYY-MM-DD}"
    The date is the current IST calendar date, so the window resets at
    midnight IST everywhere.
    ...
    def _increment(transaction: google_firestore.Transaction, ref) -> int:
        ...
        transaction.set(
            ref,
            {"email": email, "date": today_ist, "count": new_count},
            merge=True,
        )
        return new_count
