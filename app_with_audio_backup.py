...
# ── Session State Initialization & Welcome Screen ──────────────────────────
...
# Generate a new unique ID for the current turn's container
current_turn_id = f"turn-{time.time()}"

# Wrap the chat display in a container with the unique ID
with st.container():
    st.markdown(f'<div id="{current_turn_id}">', unsafe_allow_html=True)
...
if st.session_state.state["current_state"] != "END":
    ...
    if user_msg:
        # Get the ID of the container we need to hide
        id_to_hide = st.session_state.current_turn_id
        if id_to_hide:
            # Inject CSS to hide the previous turn's container INSTANTLY
            st.markdown(f'<style>#{id_to_hide} {{ display: none !important; }}</style>', unsafe_allow_html=True)
...
# This line is important: it updates the turn ID *after* the input processing block
st.session_state.current_turn_id = current_turn_id
