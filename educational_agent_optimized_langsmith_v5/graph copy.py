...
try:
    connection_kwargs = {
        ...
        # TCP keepalives: keep the socket alive so PgBouncer/Supabase don't silently drop it
        "keepalives": 1,            # Enable TCP keepalives
        "keepalives_idle": 60,      # Send first probe after 60s idle (well before PgBouncer's timeout)
    ...
    pool = ConnectionPool(
        ...
        max_idle=120,        # Recycle idle connections every 2 min (PgBouncer kills at ~5 min, this gives 3 min buffer)
