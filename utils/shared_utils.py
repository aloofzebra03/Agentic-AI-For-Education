...
def get_llm(
    ...
    if api_key is None:
        ...
        if not api_key:
            raise ValueError("No API key provided and GOOGLE_API_KEY_1 not found in environment")
...
def build_messages_with_history(
    ...
    Args:
        ...
        include_last_message: If True and state["last_user_msg"] is set, appends
            the student's latest response explicitly.
    ...
    # Assemble prompt parts using a PromptTemplate
    template_parts = ["{system_prompt}"]
    template_vars = ["system_prompt"]
